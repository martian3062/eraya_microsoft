...
const nodeStyle = (domain: string, id: string) => ({
  background: NODE_COLORS[id.split("-")[0]] ?? "#374151",
  color: "#f1f5f9",
...
export function ContextGraph({ domain }: { domain: string }) {
  ...
  useEffect(() => {
    ...
    setNodes(newNodes.map((n) => ({ ...n, style: nodeStyle(domain, n.id) })));
    setEdges(newEdges);
